// GAMEGPS — WARFRAME RESOURCE ROUTES V5.6.2
// V5.6.1 + guides précis pour les ressources raffinées quand la source est connue.
(() => {
  ...
  const routes = {
    ...
    adramalalloy: [
      {label:"SCHÉMA",path:"Deimos → Necralisk → Otak",method:"Achète le schéma réutilisable Adramal Alloy x20 auprès d'Otak. Il est disponible au rang Entrati Neutre pour 1 000 de réputation."},
      {label:"MATIÈRE PRINCIPALE",path:"Deimos → Puy de Cambion → Veines jaunes",method:"Mine les veines jaunes pour obtenir de l'Adramalium. Il peut aussi provenir de certains kystes infestés du Puy de Cambion."},
      {label:"FABRICATION",path:"Orbiteur → Fonderie → Adramal Alloy x20",method:"Pour une fabrication : 20 Adramalium + 20 Travoride + 600 Plastids + 15 Lucent Teroglobe + 1 000 crédits."}
    ]
  };
  ...
  function imageCandidates(item){const n=String(item?.imageName||'').replace(/^\/+/,''),out=[];if(n){out.push(`https://cdn.warframestat.us/img/${n}`);out.push(`https://raw.githubusercontent.com/WFCD/warframe-items/master/data/img/${n}`)}return[...new Set(out)]}
  function resourceImage(item,name){const candidates=imageCandidates(item),fallback=svgPlaceholder(name);if(!candidates.length)return`<img src="${fallback}" alt="${GPS.escape(name)}" data-gamegps-placeholder="1">`;const encoded=encodeURIComponent(JSON.stringify(candidates.slice(1)));return`<img src="${candidates[0]}" alt="${GPS.escape(name)}" data-image-fallbacks="${encoded}" data-placeholder="${encodeURIComponent(fallback)}">`}
  function armImageFallbacks(root){root?.querySelectorAll('img[data-image-fallbacks]').forEach(img=>{img.addEventListener('error',()=>{let rest=[];try{rest=JSON.parse(decodeURIComponent(img.dataset.imageFallbacks||'%5B%5D'))}catch{}if(rest.length){img.dataset.imageFallbacks=encodeURIComponent(JSON.stringify(rest.slice(1)));img.src=rest[0];return}img.removeAttribute('data-image-fallbacks');img.src=decodeURIComponent(img.dataset.placeholder||'')})})}
  ...
  GPS.renderResource=function(item){
    ...
    const cards=profiles.map((r,i)=>`<div class="wf-route"><div><span>${i===0?"OBTENTION • ":"ÉTAPE • "}${this.escape(r.label)}</span><strong>${this.escape(r.path)}</strong><small>${this.escape(r.method)}</small></div></div>`).join("");
    this.results.innerHTML=`<article class="wf-result-card"><div class="wf-result-head">${resourceImage(item,name)}<div><span class="wf-kicker">GAMEGPS / RESSOURCE V5.6.2</span><h3>${this.escape(name)}</h3><small>RESSOURCE • ${profiles.length} ÉTAPE${profiles.length>1?"S":""}</small></div></div><p class="wf-description">${this.escape(desc)}</p>${cards}${drops.length?`<div class="wf-components"><h4>SOURCES DÉTECTÉES</h4>${drops.slice(0,5).map(d=>`<div class="wf-row"><span><b>${this.escape(this.routePath(d))}</b><small>Source disponible</small></span><strong>${this.escape(this.formatChance(d))}</strong></div>`).join("")}</div>`:""}</article>`;
